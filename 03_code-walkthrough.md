...
- `set_time_limit(600)` — PHPのスクリプト実行時間制限を600秒に延長。Wikipedia巨大ページの探索は数分かかることがある
- `$absoluteDeadline` — PHPの `set_time_limit` とは別に、アプリレベルのタイムアウトを設定
...
**二重エンコード対策**: `%25E3` のように `%` 自体がエンコードされている場合（一部ブラウザの挙動）を検知して1段デコードする。
...
```php
Http::withHeaders([...])->timeout(15)->get($url, $params);
// ↓ 等価
...
**`usleep(500_000)`**: マイクロ秒単位のスリープ。`500_000` は500,000マイクロ秒 = 0.5秒。アンダースコアは数値リテラルの桁区切り（PHP 7.4+）で可読性のため。

### 6.2 ページネーション対応（`plcontinue`）
...
```php
    private function getLinks(array $titles, bool $outgoing): array
    {
        ...
        // 2. 鮮度判定
        $freshnessMap = $this->pageRepo->getFreshnessMap(array_values($idMap));

        // 3. 分類（fresh / check / stale / missing）
        $classified = $this->classifyByFreshness($idMap, $freshnessMap);

        // 4. checkグループはtouchedで確認
        if (!empty($classified['check'])) {
            $this->verifyAndRedistribute($classified);
        }

        // 5. stale + missing をAPIから取得
        ...
        // 6. DBから一括でリンクを取得
        $linkIdsByPageId = $this->linkRepo->getOutgoingTargetIds(array_values($idMap));
        // ...
    }
```
...
```php
    public function ensurePages(array $titles): array
    {
        ...
        // 既存を一括取得
        $existing = Page::whereIn('title', $titles)->pluck('id', 'title')->all();
        ...
        if (!empty($missing)) {
            ...
            $reread = Page::whereIn('title', $missing)->pluck('id', 'title')->all();
...
### 8.2 `SearchHistoryRepository::record()` — トランザクション
